...
database_integration_server <- function(){
    ...
    if (input$db_type == "PostgreSQL") {
      tryCatch({
        ...
        updateSelectInput(session, inputId = "db_schema_list",
                          choices = rv_database$schema_list,
                          selected = rv_database$schema_list[1])
      }, error = function(e) {
  ...
  observeEvent(input$option_picked, {
    if (input$option_picked == "use a table" && input$db_type == "PostgreSQL") {
      updateSelectInput(session, inputId = "db_schema_list",
                        choices = rv_database$schema_list,
                        selected = rv_database$schema_list[1])
    }
  ...
  observeEvent(input$db_schema_list, {
    ...
    if (input$db_type == "PostgreSQL") {
      ...
      query_tables <- paste0("SELECT table_name FROM information_schema.tables WHERE table_schema = '", schema_selected, "';")
      ...
      updateSelectInput(session, inputId = "db_table_list",
                        ...
                        selected = rv_database$table_list[1])
    }
  })
  ...
  observeEvent(input$db_run_query, {
    if (is.null(rv_database$conn)) {
      shinyalert(title = "", text = get_rv_labels("db_query_failure"), type = "error")
      return()
    }
    ...
    if (is.null(result)) {
      shinyalert(title = "", text = get_rv_labels("db_sql_syntax"), type = "error")
    }
  })
